...
{
	//this->fogEnable(0xe0ecff, D3DFOG_LINEAR);
...
HRESULT myIDirect3DDevice9::SetVertexShader(IDirect3DVertexShader9* pShader)
{
	if (pShader) {
		...
		if (clearCache) {
			this->vsShaderCRCs.clear();
		}

		std::map<IDirect3DVertexShader9*, std::string>::iterator it = this->vsShaderCRCs.find(pShader);
		if (it == this->vsShaderCRCs.end()) {
			UINT pSizeOfData;
			...
			char* shaderCode = static_cast<char*>(bOut->GetBufferPointer());
			//std::string disassembledStr;
			//disassembledStr.append(shaderCode);
			CRC32 crc32;
			std::string hash = crc32(pData, pSizeOfData);
			this->vsShaderCRCs.insert(std::pair<IDirect3DVertexShader9*, std::string>(pShader, hash));
		}
		it = this->vsShaderCRCs.find(pShader);
		int shadersCont = this->vsShaderCRCs.size();
		int index = -1;
		...
		if (shaderHash.compare("d643e9a1") == 0) {
			return(m_pIDirect3DDevice9->SetVertexShader(this->vsWater));
		}
...
{
	D3DXVECTOR4* value = (D3DXVECTOR4*)pConstantData;
	if (StartRegister == 19) {
		D3DXVECTOR4* fogPtr = (D3DXVECTOR4 *)pConstantData;
		D3DXVECTOR4 fogFactors = *fogPtr;
		D3DXVECTOR4 fogFactors1 = *(fogPtr+1);
	}
...
HRESULT myIDirect3DDevice9::SetPixelShader(IDirect3DPixelShader9* pShader)
{
	...
	if (pShader) {
		boolean clearCache = false;
		if (clearCache) {
			this->shaderCRCs.clear();
		}

		std::map<IDirect3DPixelShader9*, std::string>::iterator it = this->shaderCRCs.find(pShader);
		if (it == this->shaderCRCs.end()) {
			UINT pSizeOfData;
			pShader->GetFunction(NULL, &pSizeOfData);
			BYTE* pData = new BYTE[pSizeOfData];
			pShader->GetFunction(pData, &pSizeOfData);
			...
			CRC32 crc32;
			std::string hash = crc32(pData, pSizeOfData);
			this->shaderCRCs.insert(std::pair<IDirect3DPixelShader9*, std::string>(pShader, hash));
		}
		it = this->shaderCRCs.find(pShader);
		int shadersCont = this->shaderCRCs.size();
		int index = -1;
		std::string shaderHash = it->second;
		...
		if (disabledShader) {
			it = this->shaderCRCs.find(disabledShader);
...
void myIDirect3DDevice9::LoadVS(LPCWSTR file, LPDIRECT3DVERTEXSHADER9& mVShader, LPD3DXCONSTANTTABLE& mVSCTable) {
...
void myIDirect3DDevice9::LoadPS(LPCWSTR file, LPDIRECT3DPIXELSHADER9& mVShader, LPD3DXCONSTANTTABLE& mVSCTable) {
	LPD3DXBUFFER pShader = NULL;
	LPD3DXBUFFER errorBuffer = 0;
...
std::string myIDirect3DDevice9::LoadShaderFile(std::string File)
{
	std::ifstream t(File);
...
void myIDirect3DDevice9::initPipeline() {
	...
	ID3DXBuffer* avsBuffer = 0;
	ID3DXBuffer* vscErrors = 0;
	D3DXAssembleShader(vsBuffer.c_str(), vsBuffer.size(), 0, 0, 0, &avsBuffer, &vscErrors);
...
void myIDirect3DDevice9::fogEnable(DWORD Color, DWORD Mode)
{
	...
	float Density = 0.16f;   // For exponential modes
							 ...
							 // Enable fog blending.
	m_pIDirect3DDevice9->SetRenderState(D3DRS_FOGENABLE, TRUE);

	// Set the fog color.
	m_pIDirect3DDevice9->SetRenderState(D3DRS_FOGCOLOR, Color);

	// Set fog parameters.
	if (Mode == D3DFOG_LINEAR)
	{
		m_pIDirect3DDevice9->SetRenderState(D3DRS_FOGTABLEMODE, Mode);
		m_pIDirect3DDevice9->SetRenderState(D3DRS_FOGSTART, *(DWORD*)(&Start));
		m_pIDirect3DDevice9->SetRenderState(D3DRS_FOGEND, *(DWORD*)(&End));
	}
	else
	{
		m_pIDirect3DDevice9->SetRenderState(D3DRS_FOGTABLEMODE, Mode);
		m_pIDirect3DDevice9->SetRenderState(D3DRS_FOGDENSITY, *(DWORD*)(&Density));
	}
